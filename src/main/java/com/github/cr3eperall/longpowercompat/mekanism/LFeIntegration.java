...
public class LFeIntegration implements ILongFeStorage {

    private final IStrictEnergyHandler handler;

    public LFeIntegration(IStrictEnergyHandler handler) {
    ...
    public long getEnergyStoredL() {
        ...
        for (int container = 0, containers = handler.getEnergyContainerCount(); container < containers; container++) {
            long total = UnitDisplayUtils.EnergyUnit.FORGE_ENERGY.convertToAsLong(handler.getEnergy(container));
    ...
    public long getMaxEnergyStoredL() {
        long maxEnergy = 0;
        for (int container = 0, containers = handler.getEnergyContainerCount(); container < containers; container++) {
            long max = UnitDisplayUtils.EnergyUnit.FORGE_ENERGY.convertToAsLong(handler.getMaxEnergy(container));
